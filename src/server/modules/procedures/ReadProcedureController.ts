...
export class ReadProcedureController {
  async handle(request: Request, response: Response): Promise<Response> {
    try {
      const data = request.body
      const readProcedureUseCase = container.resolve(ReadProcedureUseCase)
...
@injectable()
export class ReadProcedureUseCase {
  constructor(
  ...
  async execute(data: TRequest): Promise<IProcedure[] | void> {
    const readProcedure = await this.procedureRepository.find(data)
